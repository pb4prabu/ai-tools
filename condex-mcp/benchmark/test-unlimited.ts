...
import { SafeFS, createSchema, loadProject, generateNamespace } from '../packages/condex-core/src/index.js'
...
import { loadVec, insertVectors } from '../packages/condex-core/src/retrieval/vector-search.js'
...
for (let i = 50; i <= filtered.length; i += 50) {
  ...
  if (tokens > 133735) {
    console.log(`\nCrossover: Condex exceeds naive at ~${i} results (~${tokens} tokens)`)
    break
  }
}
