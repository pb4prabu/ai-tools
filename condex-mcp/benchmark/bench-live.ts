...
import { createSchema, getRelatedSymbolIds, getSymbolsByIds } from '../packages/condex-core/src/store/sqlite-store.js'
import { loadProject } from '../packages/condex-core/src/store/loader.js'
...
import { vectorSearch, loadVec, insertVectors } from '../packages/condex-core/src/retrieval/vector-search.js'
import { getEmbedder, prepareSymbolText } from '../packages/condex-core/src/embeddings/local-embed.js'
import { indexProject } from '../packages/condex-core/src/indexer/indexer.js'
...
async function main() {
  console.log(`\n${'='.repeat(100)}`)
  console.log(`  CONDEX MCP — LIVE BENCHMARK (Threshold-based, with call-graph expansion)`)
  ...
  if (!loadResult) {
    console.error('  ERROR: No index found. Run: condex index --full')
    process.exit(1)
  }
  ...
  console.log(`${'='.repeat(100)}`)
  console.log(``)
  console.log(`  WITHOUT CONDEX (Naive — read matching source files)`)
  console.log(`    Tokens: ${totals.naive.toLocaleString()}`)
  console.log(`    Time:   ${totals.naiveTime.toFixed(0)}ms total`)
  console.log(``)
  console.log(`  CONDEX BM25 (score threshold ≥ ${BM25_MIN_SCORE}, + call-graph expansion)`)
  ...
  if (VECTOR_ENABLED) {
    console.log(``)
    console.log(`  CONDEX VECTOR (distance threshold ≤ ${VECTOR_MAX_DISTANCE})`)
    ...
    console.log(``)
    console.log(`  CONDEX HYBRID (BM25 + Vector, RRF + call-graph)`)
    ...
    console.log(``)
    console.log(`  CONDEX SMART (BM25 ≥ ${SMART_BM25_MIN_SCORE} first, vector ≤ ${SMART_VECTOR_MAX_DISTANCE} fallback) ★ RECOMMENDED`)
