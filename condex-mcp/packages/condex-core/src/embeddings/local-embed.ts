 ...
 *
 * Setup: Run `bash setup-model.sh` to download model via curl,
 * or `npm run download-model` to download via HuggingFace JS SDK.
 */
...
export function getCacheDir(): string {
  ...
  // Default: ~/.condex/models/ — persists across reboots (unlike /tmp).
  const home = process.env.HOME || process.env.USERPROFILE || '/tmp'
  return path.join(home, '.condex', 'models')
}

/**
 * Ensure the model is available in the cache directory.
 * If not present, downloads from HuggingFace (~100MB).
 * For offline setup, run `bash setup-model.sh` first.
 */
