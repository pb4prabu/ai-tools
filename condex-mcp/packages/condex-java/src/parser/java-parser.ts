...
import type { Symbol, SymbolKind, SymbolRef, RefType } from '@condex-ai/core'
import { buildSymbolId } from '@condex-ai/core'
...
export function parseJavaFileWithRefs(content: string, opts: ParseOptions): ParseResult {
  ...
  try {
    ...
    console.error(`[condex-java] Failed to parse: ${opts.filePath}`)
    return { symbols: [], refs: [] }
  }
