...
**Path:** `packages/condex-core/src/server.ts`

## What it does

This is the main executable that starts the Condex MCP server. It's what gets launched when an AI agent (like Claude Code or OpenCode) spawns the MCP process.
...
5. Load Java parser (optional, from @condex-ai/java)
6. Auto-index:
   - If .condex/index/meta.json exists → load existing index
...
- **In-memory SQLite:** The database lives entirely in RAM. Fast, but lost when the process exits. The `.condex/index/` on disk is the durable store.
...
|----------|--------|---------|--------|
| `CONDEX_SEARCH_MODE` | `bm25`, `vector`, `hybrid`, `smart` | `bm25` | Controls which search strategy is used |
