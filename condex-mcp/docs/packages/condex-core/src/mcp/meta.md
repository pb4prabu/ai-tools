...
**Path:** `packages/condex-core/src/mcp/meta.ts`

## What it does

Builds the `_meta` envelope attached to every MCP tool response. This metadata tells the AI agent how much it saved by using Condex instead of reading raw files.
