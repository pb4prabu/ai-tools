# Condex MCP — Complete Project Plan
...
Condex is a **local-first, project-namespaced MCP server** that indexes a codebase once and lets AI agents (OpenCode, Claude Code, etc.) retrieve only the exact symbols they need — instead of reading entire files.
...
**Non-objective (explicitly out of scope for v1)**: Context tree / call graph analysis. The AI model handles relationship reasoning. Condex only handles symbol retrieval.
...
- **Condex will never be confidently wrong.** A confidence gate fires before returning results. If score < threshold, it returns nothing and the agent falls back to normal file reading.
...
- **The AI always has fallback.** Condex is an accelerator, not a gatekeeper. If Condex returns nothing, OpenCode uses its own file search tools unchanged.
...
┌────────────────────────┐      ┌──────────────────────┐
│     condex-java        │      │    condex-python      │ (future)
│                        │      │                       │
...
│   unknown)             │
│  Flyway SQL parser     │      ┌──────────────────────┐
│  application.yml       │      │    condex-auto        │ (future)
│  OpenAPI annotations   │      │                       │
...
```
condex/                                   ← GitHub: github.com/yourorg/condex
├── packages/
│   ├── condex-core/                      ← @condex-ai/core
│   │   ├── src/
...
│   │   └── tsconfig.json
│   │
│   └── condex-cli/                       ← condex (global CLI command)
│       ├── src/
...
When a tool call arrives, Condex detects the project root by walking up the directory tree from the given path, looking for these markers in priority order:

```
1. .condex/condex.config.json    ← explicit Condex config (highest priority)
...
Everything in `.condex/index/` is committed to git. The SQLite database is never committed — it is always rebuilt at startup.
...
└── .condex/
    ...
    ├── condex.config.json                ← committed, project config
    └── index/
...
Built in memory (or `~/.condex/runtime.db`) at startup from filesystem files. Never committed. Discarded when MCP server stops.
...
Every language extension implements this interface. `condex-core` calls it without knowing the language.

```typescript
// condex-core/src/types/parser.interface.ts
...
// Store model in ~/.condex/models/ rather than node_modules cache
env.cacheDir = path.join(os.homedir(), '.condex', 'models')
...
All defined in `condex-core`. All include `_meta` envelope with token savings.

```typescript
// tools.ts

export const CONDEX_TOOLS = [

  {
    name: 'index_folder',
    description: 'Index a project folder. Detects language and architecture automatically. Writes index to .condex/index/ in the project.',
    inputSchema: {
      type: 'object',
      properties: {
...
export class SavingsTracker {
  ...
  constructor(projectRoot: string) {
    this.savingsPath = path.join(projectRoot, '.condex', 'savings.json')
...
// Naive token estimate — what the AI would have read without Condex
...
import { CONDEX_TOOLS } from './mcp/tools.js'
...
async function main() {
  console.error('🧠 Condex MCP Server starting...')
  ...
  const indexRoot = process.env.CONDEX_INDEX_PATH ||
    path.join(os.homedir(), '.condex', 'indexes')
...
### condex-java/package.json

```json
{
  "name": "@condex-ai/java",
  "version": "1.0.0",
  "description": "Java parser for Condex — Spring-aware, architecture-detecting",
  "type": "module",
  "main": "dist/index.js",
  "dependencies": {
    "@condex-ai/core": "workspace:*",
    "tree-sitter": "^0.21.0",
...
```json
{
  ...
  "private": true,
  "workspaces": [
    "packages/condex-core",
    "packages/condex-java",
    "packages/condex-cli"
  ],
  "scripts": {
...
```json
{
  "mcpServers": {
    "condex": {
      ...
      "args": [],
      "env": {
        "CONDEX_INDEX_PATH": "${workspaceFolder}/.condex"
      }
    }
  }
}
...
```json
{
  "mcpServers": {
    "condex": {
      "type": "stdio",
      "command": "node",
      "args": ["/path/to/condex-core/dist/server.js"],
      "env": {
        "CONDEX_INDEX_PATH": "${workspaceFolder}/.condex",
        "CONDEX_LANGUAGE": "java"
      }
    }
  }
}
...
# Commit index
git add .condex/
git commit -m "condex: update index"
git push
```
...
# Then:
git submodule add https://github.com/yourorg/myapp-condex-index .condex
condex index .
cd .condex
git add -A
git commit -m "condex: initial index"
git push
cd ..
git add .condex
git commit -m "add condex index as submodule"
git push

# Team members:
git clone --recurse-submodules https://github.com/yourorg/myapp
npm install -g @condex-ai/core @condex-ai/java
# Start OpenCode — condex loads automatically
```

### After code changes:

```bash
condex index .                              # incremental, ~3 seconds
cd .condex && git add -A && git commit -m "condex: update index" && git push
cd .. && git add .condex && git commit -m "bump condex index"
```
...
**Deliverable**: MCP server starts, loads `.condex/index/` into SQLite, `list_projects` tool works.
...
**Deliverable**: OpenCode can search symbols via Condex. Test manually on your monorepo.
...
**Deliverable**: `condex index .` works on your Java monorepo. Symbols are correctly extracted and tagged.
...
- Implement `condex-cli` with `index`, `list`, `status`, `invalidate` commands
- `npm publish @condex-ai/core` + `@condex-ai/java`
...
| Task | Without Condex | With Condex | Saving |
|---|---|---|---|
...
> Note: "Without Condex" = sum of raw file tokens that would have been read by the agent to answer the same question. Measured from OpenCode session logs.
...
| Package distribution | npm (@condex-ai/core, @condex-ai/java) | Easy global install, easy update |
...
*End of Condex MCP Project Plan v1.0*
