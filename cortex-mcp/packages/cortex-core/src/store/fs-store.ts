...
import type { CortexConfig, ProjectMeta, DEFAULT_CONFIG } from '../types/config.js'

const TOOL_VERSION = '1.0.0'

const CORTEX_GITIGNORE = `# Never commit runtime files
*.db
*.db-shm
*.db-wal
savings.json
...
/**
 * Initialize .cortex/ directory structure inside project root.
 */
export async function initCortexDir(safeFs: SafeFS): Promise<void> {
  const cortexDir = safeFs.getCortexDir()
  ...
  await safeFs.writeFile(path.join(cortexDir, '.gitignore'), CORTEX_GITIGNORE)
}
...
export async function readSymbolFiles(safeFs: SafeFS): Promise<Symbol[]> {
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.error(`[cortex] Warning: skipping malformed symbol file: ${file}`)
    }
  }
...
export async function readCortexConfig(safeFs: SafeFS): Promise<CortexConfig | null> {
  const filePath = path.join(safeFs.getCortexDir(), 'cortex.config.json')
  if (!await safeFs.exists(filePath)) return null
  try {
    const content = await safeFs.readFile(filePath)
    return JSON.parse(content) as CortexConfig
  } catch {
    return null
  }
}

export async function writeCortexConfig(
  safeFs: SafeFS,
  config: CortexConfig
): Promise<void> {
  const filePath = path.join(safeFs.getCortexDir(), 'cortex.config.json')
...
export async function acquireIndexLock(safeFs: SafeFS): Promise<boolean> {
  const lockPath = path.join(safeFs.getCortexDir(), '.indexing.lock')
  ...
  if (await safeFs.exists(lockPath)) {
    try {
      ...
      console.error(`[cortex] Clearing stale index lock (age: ${Math.round(lockAge / 1000)}s)`)
    } catch {
