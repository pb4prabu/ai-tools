...
export function blockOutboundNetwork(): void {
  ...
  const throwBlocked = (): never => {
    throw new Error(
      'NETWORK_BLOCKED: Cortex MCP server does not allow outbound network connections. ' +
      'This is a security feature. To download models, use "cortex setup --vector" instead.'
    )
  }
  ...
  console.error('[cortex] Network guard active — all outbound connections blocked')
}
...
export function generateSandboxProfile(projectRoot: string): string {
  return [
    ...
    // Block all file writes except .cortex/ and temp
