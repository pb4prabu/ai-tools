...
**Path:** `packages/cortex-core/src/types/`
...
### config.ts
- `CortexConfig` — Project config from `cortex.config.json` (language, include/exclude globs, retrieval settings)
...
### meta.ts
- `CortexMeta` — The `_meta` envelope in every MCP response (timing, token counts, savings, gate status)
