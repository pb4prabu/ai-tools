...
**Path:** `packages/cortex-core/src/security/fs-guard.ts`
...
- **Writes:** Only allowed inside `.cortex/` directory
...
| `writeFile(path, data)` | Write | Must be within `.cortex/` |
| `mkdir(path)` | Write | Must be within `.cortex/` |
| `rm(path)` | Write | Must be within `.cortex/` |
...
safeFs.writeFile('/home/user/my-project/.cortex/data')   // OK
