#!/usr/bin/env npx tsx
/**
 * 3-Way Benchmark: Naive vs Cortex BM25 vs Cortex BM25+Vector
 *
...
// Use the cortex-core source directly (via tsx)
import { SafeFS } from '../packages/cortex-core/src/security/fs-guard.js'
import { createSchema, insertProject, insertSymbols, getSymbolById } from '../packages/cortex-core/src/store/sqlite-store.js'
...
// Java parser
import { parseJavaFile } from '../packages/cortex-java/src/parser/java-parser.js'
import { detectArchitecture } from '../packages/cortex-java/src/parser/arch-detector.js'
...
function findJavaFiles(dir: string, base: string = ''): string[] {
  ...
  for (const entry of fs.readdirSync(path.join(dir, base), { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      if (entry.name === '.cortex' || entry.name === 'node_modules') continue
...
// ─── Phase 2: Naive Approach (No Cortex) ───────────────────

/**
 * Simulate what an LLM would do without Cortex:
...
async function initVector(db: Database.Database, allSymbols: any[]) {
  ...
  const { getEmbedder, prepareSymbolText } = await import(
    '../packages/cortex-core/src/embeddings/local-embed.js'
  )
...
async function runBenchmark() {
  console.log('╔══════════════════════════════════════════════════════════════════════════════╗')
  console.log('║                    Cortex MCP — Token Savings Benchmark                     ║')
  console.log('╚══════════════════════════════════════════════════════════════════════════════╝')
  ...
  const approaches = [
    { name: 'Without Cortex (Naive)', total: totalNaive, saved: 0, pct: 0, note: 'read all matching source files' },
    { name: 'Cortex BM25 Only', total: totalBM25, saved: totalNaive - totalBM25, pct: bm25SavPct, note: `gate fires on ${results.filter(r => r.bm25.gateFired).length}/${QUERIES.length} queries` },
  ]
  if (VECTOR_ENABLED) {
    approaches.push(
      { name: 'Cortex BM25+Vector Hybrid', total: totalVector, saved: totalNaive - totalVector, pct: vecSavPct, note: 'always returns results, more verbose' },
      { name: 'Cortex Smart (BM25 + vec fallback)', total: totalSmart, saved: totalNaive - totalSmart, pct: smartSavPct, note: `vector fallback on ${results.filter(r => r.smart?.usedVector).length}/${QUERIES.length} queries` },
    )
  }
