# Cortex MCP — Execution Plan & Status Tracker
...
4. **Index path inconsistency** — `server.ts` loads from `~/.cortex/indexes/` but the filesystem layout stores in `{project}/.cortex/index/`. Resolution: indexes live INSIDE the project at `.cortex/index/`. The server discovers projects by receiving paths via tool calls. No central `~/.cortex/indexes/` directory.

**DESIGN SIMPLIFICATION (user decision):**
- **Project root = `process.cwd()`**. When OpenCode/Claude Code starts Cortex as a child process, CWD is inherited. That's the project root. No config, no env vars, no arguments needed.
...
- When scanning files for indexing, **skip any `.cortex/` subdirectories** — a child project's index is invisible to the parent.
- No scope bias, no marker priority walking, no parent/child conflict resolution needed.

**ZERO-CONFIG INTEGRATION (user decision):**
- **One-time setup**: Add Cortex to OpenCode/Claude Code global MCP config (not per-project).
- **Every session**: User opens OpenCode in any folder → Cortex auto-starts, auto-indexes, tools ready.
- **No per-project config required**. `.cortex/cortex.config.json` is optional (for overrides like `vectorSearch: true`, custom include/exclude globs). Without it, Cortex auto-detects language and uses sensible defaults.
...
- **On MCP server startup**: Cortex immediately indexes from CWD so tools are ready before the AI's first query.
  1. `.cortex/index/meta.json` exists? → incremental index (~50ms if nothing changed)
  2. No `.cortex/` at all? → full index from scratch (~3-8s), creates `.cortex/`
...
- If `cortex.config.json` has `"vectorSearch": true` → enable hybrid BM25 + vector search.
- When enabled: downloads `nomic-ai/nomic-embed-text-v1.5` (~30MB) on first use, cached at `~/.cortex/models/`. All inference is local (ONNX on CPU). No data ever leaves the machine.
- Config in `.cortex/cortex.config.json`:
  ```json
  {
...
9. **No concurrency control** — Two simultaneous `index_folder` calls can corrupt the index. Fix: use a simple lockfile (`.cortex/.indexing.lock`) with PID + timestamp. Stale lock (>5 min) auto-clears.
...
16. **Start with TypeScript parser alongside Java** — We're writing the tool in TypeScript, so we can dogfood it immediately. Add a minimal TS parser in Phase 4b. This lets us test against Cortex's own codebase.
...
18. **3-LAYER OUTBOUND NETWORK BLOCK (user decision — security guarantee):**
    Only the Cortex MCP server process is blocked. All other processes on the machine are unaffected.
    ...
    **Model download is separate:**
    - `cortex setup --vector` CLI command downloads model (runs WITHOUT sandbox, network allowed)
    - MCP server never downloads anything — if model missing, throws error: "Run `cortex setup --vector` first"
...
19. **FILESYSTEM ACCESS RESTRICTED TO PROJECT ROOT ONLY (user decision — security guarantee):**
    ...
    **Layer 1 — Application-level SafeFS guard (in-code):**
    - All filesystem operations in Cortex go through `SafeFS` wrapper
    - `SafeFS` validates every path is within `projectRoot` (reads) or `projectRoot/.cortex` (writes)
    ...
    **Layer 2 — sandbox-exec filesystem restrictions (OS-level, macOS):**
    - Extend the sandbox profile to restrict file-write to ONLY `{projectRoot}/.cortex/` + system temp
    - Combined with network block in one profile:
      ...
      (allow file-write* (subpath "{projectRoot}/.cortex"))
    ...
    **Layer 3 — Node.js --experimental-permission (optional, strictest reads):**
    - Restricts even READ access to only specified paths:
      ...
      node --experimental-permission \
           ...
           --allow-fs-write="${PROJECT_ROOT}/.cortex" \
           --allow-fs-read="${HOME}/.cortex/models" \
           dist/server.js
      ```
    ...
    **MCP config (simple — CWD is inherited automatically):**
    ...
    ```
    The `cortex-mcp` launcher script:
    ...
    **What CAN be accessed:**
    ...
    - ✅ `{projectRoot}/.cortex/**` — read/write index
    - ✅ `~/.cortex/models/` — read cached embedding model (vector mode only)
    - ✅ Node.js runtime paths — required for process to run

    **What CANNOT be accessed:**
    - ❌ Any other folder on the machine
    - ❌ Home directory (except ~/.cortex/models/)
...
- [ ] 0.3 — Create `packages/cortex-core/` with `package.json`, `tsconfig.json`, `src/` dir
- [ ] 0.4 — Create `packages/cortex-java/` with `package.json`, `tsconfig.json`, `src/` dir
- [ ] 0.5 — Create `packages/cortex-cli/` with `package.json`, `tsconfig.json`, `src/` dir
...
- [ ] 0.8 — Create `.gitignore` (node_modules, dist, *.db, .cortex/*.db, savings.json)
- [ ] 0.9 — Implement `cortex-core/src/security/network-guard.ts`
  ...
  - **Layer 2**: `blockOutboundNetwork()` — monkey-patches ALL Node.js networking APIs:
    ...
    - All patched to throw `Error('NETWORK_BLOCKED: Cortex does not allow outbound connections')`
...
- [ ] 0.10 — Implement `cortex-core/src/security/fs-guard.ts`
  ...
  - Writes restricted to `{projectRoot}/.cortex/` only
  - Reads restricted to `{projectRoot}/` only
  - Any violation throws `Error('FS_BLOCKED: Access denied outside project root')`
  - All file operations in Cortex use SafeFS instead of raw `fs` module
- [ ] 0.11 — Implement `cortex-core/src/security/sandbox-profile.ts`
...
- [ ] 0.12 — Implement `cortex-mcp` launcher script (bin entry)
...
- [ ] 0.13 — Copy original plan to `docs/CORTEX_MCP_PLAN.md` for reference
- [ ] 0.10 — Copy original plan to `docs/CORTEX_MCP_PLAN.md` for reference
...
- [ ] 1.1 — Create `cortex-core/src/types/symbol.ts` — Symbol, SymbolKind, SpringRole, HexRole types
- [ ] 1.2 — Create `cortex-core/src/types/parser.ts` — CoreParser interface, ParseResult, ProjectProfile
- [ ] 1.3 — Create `cortex-core/src/types/schema.ts` — SchemaSymbol, ConfigSymbol types
- [ ] 1.4 — Create `cortex-core/src/types/retrieval.ts` — BM25Result, VectorResult, FusedResult, GateResult, SearchFilters
- [ ] 1.5 — Create `cortex-core/src/types/config.ts` — CortexConfig type (no RootDetectionResult needed — path IS root)
- [ ] 1.6 — Create `cortex-core/src/types/meta.ts` — CortexMeta response envelope type
- [ ] 1.7 — Create `cortex-core/src/types/index.ts` — barrel export all types
- [ ] 1.8 — Write basic type tests (vitest type-checking)

**Deliverable**: All interfaces defined, importable from `@cortex-ai/core`.
...
- [ ] 2.1 — Implement `cortex-core/src/namespace/namespace.ts`
...
**Goal**: Read/write `.cortex/index/` directory structure.

- [ ] 3.1 — Implement `cortex-core/src/store/fs-store.ts`
  - Uses `SafeFS` for ALL file operations — never uses raw `fs` module
  - `initCortexDir(projectRoot)` — creates `.cortex/index/symbols/`, `.cortex/index/schema/`, `.cortex/index/config/`, `.cortex/.gitignore`
  - All file scanning functions **always exclude `.cortex/` directories** — child project indexes are invisible to parent
  ...
  - `writeCortexConfig(projectRoot, config)` / `readCortexConfig(projectRoot)`
- [ ] 3.2 — Implement lockfile mechanism
  - `acquireIndexLock(projectRoot)` — creates `.cortex/.indexing.lock` with PID + timestamp
...
- [ ] 4.2 — Implement `cortex-core/src/store/sqlite-store.ts`
...
- [ ] 4.3 — Implement `cortex-core/src/store/loader.ts`
  - `loadProject(db, projectRoot)` — reads .cortex/index/, inserts into SQLite
  - `loadProjectFromDir(db, cortexDir)` — lower-level loader
...
- [ ] 5.1 — Implement `cortex-core/src/retrieval/bm25.ts`
...
- [ ] 5.2 — Implement `cortex-core/src/retrieval/confidence-gate.ts`
...
- [ ] 6.2 — Implement `cortex-core/src/server.ts`
  ...
  - **On startup (before accepting tool calls):**
    ...
    2. Run index: `.cortex/` exists → incremental | missing → full index
    3. Load index into in-memory SQLite
    4. Log: "Cortex ready: {symbolCount} symbols, {architecture}, indexed in {ms}ms"
- [ ] 6.3 — Implement `cortex-core/src/mcp/tools.ts` — all 10 tool definitions (schema only)
- [ ] 6.4 — Implement `cortex-core/src/mcp/dispatcher.ts` — routes tool name to handler function
- [ ] 6.5 — Implement `cortex-core/src/mcp/meta.ts` — `buildMeta()` helper for _meta envelope
- [ ] 6.6 — Implement auto-index middleware
  - Before any query tool runs, call `ensureIndex(projectRoot, db)`
  - `ensureIndex` checks `.cortex/index/meta.json`:
    - Exists → run incremental (compare hashes, re-parse changed files only)
    - Missing → run full index, create `.cortex/`
...
- [ ] 6.7 — Implement tool handlers (BM25 only):
  ...
  - `invalidate_cache` — delete .cortex/index/ for project
...
- [ ] 7.3 — Implement `cortex-java/src/parser/spring-tagger.ts`
...
- [ ] 7.4 — Implement `cortex-java/src/parser/arch-detector.ts`
...
- [ ] 7.5 — Implement `cortex-java/src/parser/hex-role-tagger.ts`
  - `assignHexRole(symbol, profile)` → HexRole
  - Only active when architecture === 'hexagonal'
- [ ] 7.6 — Implement `cortex-java/src/index.ts` — export JavaParser implementing CoreParser
- [ ] 7.7 — Implement `index_folder` tool handler in cortex-core
  - The given path IS the project root — no walking up
  - Detect language from config or file extensions
  - Load appropriate parser (cortex-java for .java files)
  - Parse all files matching include globs, **always exclude `**/.cortex/**`** (hardcoded, non-overridable)
  - Write symbol files to .cortex/index/symbols/
  - Write meta.json
  - Detect architecture
...
**Deliverable**: `cortex index .` works on a Java project. Symbols correctly extracted + tagged.
...
- [ ] 8.2 — Implement `cortex-core/src/token/counter.ts`
  - `countTokens(text)` using cl100k_base tokenizer
- [ ] 8.3 — Implement `cortex-core/src/token/savings.ts`
  - `SavingsTracker` class (renamed method: `trackSaving()`)
  - Persist to `.cortex/savings.json`
...
- [ ] 9.2 — Implement `cortex-core/src/embeddings/local-embed.ts`
  - Use `@huggingface/transformers` v3 API (pipeline, feature-extraction)
  - Model: `nomic-ai/nomic-embed-text-v1.5` (quantized, ~30MB, cached at `~/.cortex/models/`)
  ...
  - Guard: if model not downloaded → throw error: "Run `cortex setup --vector` first"
...
## Phase 11: CLI (cortex-cli)
**Goal**: Global CLI for manual index management.

- [ ] 11.1 — Implement `cortex-cli/src/cli.ts`
  - `cortex index [path]` — triggers index_folder (auto-detect language, incremental default)
  - `cortex index [path] --full` — force full re-index
  - `cortex index [path] --language java`
  - `cortex list` — list all indexed projects (scan home dir for .cortex/)
  - `cortex status` — current project index stats
  - `cortex invalidate [path]` — remove index
  - `cortex setup --vector` — downloads embedding model to `~/.cortex/models/` (ONLY command with network access, does NOT call blockOutboundNetwork)
- [ ] 11.2 — Add shebang + bin entry in package.json
- [ ] 11.3 — Tests: CLI argument parsing, command routing

**Deliverable**: `cortex index .` works from terminal.
...
Rule 2: Index lives at {projectRoot}/.cortex/index/
Rule 3: Namespace = folderName@sha256(absolutePath)[0:6]
Rule 4: When scanning files, ALWAYS skip **/.cortex/** (hardcoded)
Rule 5: Parent project never sees child's .cortex/ — they are invisible to each other
...
### What's Built
- **cortex-core**: Security guards, namespace, FS store, SQLite store + FTS5, BM25 search, confidence gate, indexer, MCP server (10 tools), token tracking, savings persistence
- **cortex-java**: Tree-sitter Java parser, Spring role tagger, architecture detector (hex/layered/MVC), hex role tagger, SQL migration parser, YAML config parser
- **cortex-cli**: `cortex index`, `cortex status`, `cortex invalidate` commands
