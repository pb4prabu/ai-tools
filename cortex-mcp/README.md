...
Cortex indexes your codebase into a searchable symbol database and exposes it via the [Model Context Protocol](https://modelcontextprotocol.io). Instead of AI agents reading entire source files, they query Cortex for precisely the symbols they need — saving 85-92% of context tokens.

```
Without Cortex:  Agent reads 12 files     → 80,501 tokens
With Cortex:     Agent queries 4 symbols  →  6,588 tokens (91.8% saved)
```
...
|---------|---------|-----------------|
| **@cortex-ai/core** | MCP server, retrieval, storage, security | `@modelcontextprotocol/sdk`, `better-sqlite3`, `sqlite-vec`, `@huggingface/transformers` |
| **@cortex-ai/java** | Spring-aware Java parser with architecture detection | `tree-sitter`, `tree-sitter-java`, `js-yaml` |
| **@cortex-ai/cli** | CLI for manual indexing and management | `@cortex-ai/core`, `@cortex-ai/java` |
...
Controlled by `CORTEX_SEARCH_MODE` environment variable:

### BM25 (default)

```
CORTEX_SEARCH_MODE=bm25
```
...
- **First run**: Downloads ~100MB model to `~/.cortex/models/`
...
Cortex exposes 10 tools via the Model Context Protocol:
...
Override in `.cortex/cortex.config.json`:
```json
{
  "retrieval": {
...
Cortex is designed to be **zero-trust**: no network access, restricted filesystem, sandboxed execution.
...
Any network call throws: `NETWORK_BLOCKED: Cortex MCP server does not allow outbound network connections.`
...
After the embedding model is cached locally (`~/.cortex/models/`), vector/hybrid modes operate fully offline.

### Filesystem Guard (SafeFS)

All file operations go through `SafeFS`:
- **Read**: Only within project root
- **Write**: Only within `.cortex/` directory
...
Cumulative savings persisted in `.cortex/savings.json`.
...
|----------|--------|---------|---------|
| `CORTEX_SEARCH_MODE` | `bm25` / `vector` / `hybrid` | `bm25` | Search algorithm |
...
```json
{
  ...
  "mcp": {
    "cortex-bm25": {
      "type": "local",
      "command": ["node", "/path/to/cortex-mcp/packages/cortex-core/dist/server.js"],
      "environment": { "CORTEX_SEARCH_MODE": "bm25" },
      "enabled": true
    },
    "cortex-vector": {
      "type": "local",
      "command": ["node", "/path/to/cortex-mcp/packages/cortex-core/dist/server.js"],
      "environment": { "CORTEX_SEARCH_MODE": "vector" },
      "enabled": false
    },
    "cortex-hybrid": {
      "type": "local",
      "command": ["node", "/path/to/cortex-mcp/packages/cortex-core/dist/server.js"],
      "environment": { "CORTEX_SEARCH_MODE": "hybrid" },
      "enabled": false
    }
  }
}
...
```json
{
  "cortex-mcp": {
    "type": "stdio",
    "command": "node",
    "args": ["/path/to/cortex-mcp/packages/cortex-core/dist/server.js"]
  }
}
```
...
├── cortex.config.json     # Optional project config
...
node /path/to/cortex-mcp/packages/cortex-core/dist/server.js
```

The server:
1. Detects project root from `process.cwd()`
2. Loads existing index from `.cortex/index/` (or runs full index if Java parser available)
...
| Scenario | Behavior |
|----------|----------|
| No `.cortex/index/` + Java parser available | Full index (3-8s for ~800 files) |
| No `.cortex/index/` + no Java parser | Error: "No parser available and no existing index" |
| `.cortex/index/` exists | Load from disk (~1s for ~2700 symbols) |
